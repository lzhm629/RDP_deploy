...
def load_trusted_torch_checkpoint(path, map_location=None):
    try:
        return torch.load(
            path,
...
class DecoderRNN(nn.Module):
    def __init__(
        ...
        temporal_cond_dim = int(temporal_cond_dim)
        output_dim = int(output_dim)
        hidden_dim = int(hidden_dim)
        layer_num = int(layer_num)
...
class VAE:
    def __init__(
        ...
        self.input_dim_h = int(horizon)
        self.input_dim_w = int(shape_meta['action']['shape'][0])
        n_latent_dims = int(n_latent_dims)
        mlp_layer_num = int(mlp_layer_num)
        conv_latent_dims = int(conv_latent_dims)
        conv_layer_num = int(conv_layer_num)
        rnn_latent_dims = int(rnn_latent_dims)
        rnn_layer_num = int(rnn_layer_num)
        n_embed = int(n_embed)
        vqvae_groups = int(vqvae_groups)
        ...
        if self.use_rnn_decoder:
            ...
            self.rnn_temporal_cond_dim = int(sum([
                int(shape_meta['extended_obs'][extented_obs_key]['shape'][0])
        ...
        if len(output_shape) == 1:
            ...
            decoder_n_latent_dims = int(np.multiply(*output_shape))
            self.downsampled_input_h = int(output_shape[0])
        ...
        if load_dir is not None:
            try:
                state_dict = load_trusted_torch_checkpoint(load_dir)
            except RuntimeError:
                state_dict = load_trusted_torch_checkpoint(load_dir, map_location=torch.device("cpu"))
